...
注意到`phase_1()`函数中的`  400ee4:	be 00 24 40 00       	mov    $0x402400,%esi`,

通过阅读`strings_not_equal()`函数的汇编代码可知，其两个参数的地址分别放在`rdi`和`rsi`寄存器中，

而`read_line()`的返回值被放在了`rdi`中,由此可知,`esi`中存放的就是将要和我们的输入比较的字符串在内存中的地址`0x402400`。
